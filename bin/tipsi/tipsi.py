...
options = sparser.Options([
    ...
    (  "-X",        "coords",         str,    1,      None,     True,   "Coordinate sets (PDB/GRO/TPR/TRR): -X ref1=file.gro"),
...
class Phase(collections.MutableMapping):
    ...
    def __eq__(self,other):
        if (other == None or self.time != other.time or self.file != other.file or self.offset != other.offset):
            return False
        return True
    ...
    def short(self):
        return [self.file, self.time, self.state, self.offset]
...
class Path(collections.Sequence):
    ...
    def __str__(self):
        return "%s  ==>  %s"%(" ".join(self[0].short())," ".join(self[-1].short()))
...
class TPS:
    __tags = (
        "init",         # Initialization expressions for calculator       
        "expressions",  # Expressions for calculator for every frame
        "parameters",   # Parameter expressions (saved to output file)
        "print",        # Variables to print for every frame
        "write",        # Variables to write to file for every frame
        "states",       # State expressions
        "interfaces",   # Interface expressions
        "bias",         # Bias expression for selection of shooting point
        "recrossing",   # Recrossing rules (redundant, I think)
        "coords",       # Coordinate sets
    )
    __par = (
...
def do_trajectory(trj, options, par, offset=None):
    ...
    # Read and register coordinate sets
    coords = [[j.strip() for j in i.split("=")] for i in par.coords]
    for var, filename in coords:
        fnm = filename.lower()
        if (fnm.endswith(".gro") or 
            fnm.endswith(".pdb") or
            fnm.endswith(".gro.gz") or 
            fnm.endswith(".pdb.gz")):
            coords = [ i for i in gmx.open(filename) ]
            if len(coords) == 1:
                coords = coords[0]
        elif fnm.endswith(".trr") or fnm.endswith(".trr.gz"):
            coords = [ i.x() for i in gmx.open(filename) ]
            if len(coords) == 1:
                coords = coords[0]
        elif fnm.endswith(".tpr") or fnm.endswith(".tpr.gz"):
            coords = gmx.open(filename.strip()).x
        print type(coords)
        calc[var.strip()]=coords
    ...
    while processIsActive:
        tpath, position = process_trajectory(trj, position, calc, par, options.pid, not options.verbose)
        path.extend(tpath)
        nframes = len(tpath)
...
def find_transitions(path, options, par):
    ...
    for segment in path.segments("state"):
        if (segment[1]["state"] in interfaces and
            ...
            newpath = path[segment[0]:segment[2]]
            if len(newpath) >= options.minpathlength:
                transitions.append(newpath)
...
class ShootingPoint:
    def __init__( self, tpr, path, margin, bias ):
        ...
        if self.v is None:
            print "No velocities found in trajectory. Inferring velocities from displacements."
            print "This probably happens only once, for the initial run."
            self.v = (trj.next().x() - self.x)/self.dt
        elif not self.tpr.get('integrator'):
            ...
            # The default integrator uses the Leap-Frog algorithm (md), which stores velocities 
            # at half time steps. The enum index of the default integrator is 0.
            print ("\nWARNING: This tpr uses the Leap-Frog integrator. With Velocity-Verlet"
                   "positions and velocities are both available at whole time steps.")
...
def main(argv=None):
    ...
    if mode == "check":
        # CHECK THIS!!!
        trans     = find_transitions(Path(*options.pathinfiles), options, par)
        if trans == []:
            print "\nNo transition in file.\n"
            exit_code = 1
        else:
            print "\nFound %d transition%s in input:\n"%(len(trans),(len(trans)>1)*"s")
            for i in trans:
                print "\t",i
            print
            exit_code = 0

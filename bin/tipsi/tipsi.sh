...
STUFF="$STUFF -maxf $TPS_MXF -2way $TPS_2WY -rand $TPS_RND -minlen $TPS_MIN $TPS_VRB"
...
function run_md()
{
    ...
    GROMIT="$SDIR/gromit.sh -f $tpr --mdrun-append -np $NP -g ${tpr%.tpr}.out -e ${tpr%.tpr}.err $GMXRC"
    CONTROL="$MONIT -out ${tpr%.tpr}.dat"
    echo $GROMIT -control $CONTROL -mdrun $MDRUN
    $GROMIT -control "$CONTROL" -mdrun "$MDRUN"
    OK=$?

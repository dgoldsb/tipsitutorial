...
few less ordinary commands. Unfortunately, some of these are specific to GNU sed, 
which causes this program to fail on Mac. This should be fixed...
...
NAME=         # Run name
TOP=          # Topology file
FETCH=false   # Try to fetch PDB file from web
...
MDRUN=        # mdrun executable (or alias)
...
# Set the directory for Gromacs binaries
# Try to find mdrun 
GMXBIN=$(which mdrun)
...
[[ -x ${MDRUN%% *}   ]] || chmod +x ${MDRUN%% *}


# Find out which Gromacs version this is
GMXVERSION=$(editconf -h 2>&1 1>/dev/null | sed -n 's/ *:-)  VERSION \(.*\)  (-: */\1/p')
...
__mdp_amber__coulombtype=PME-switch
__mdp_amber__rcoulomb=1.1
__mdp_amber__rlist=1.2
__mdp_amber__vdwtype=shift
__mdp_amber__rvdw=1.0
__mdp_amber__rvdw_switch=0.9
...
function MDRUNNER ()
{
	...
	GROMPP="$GMXBIN/grompp $fnMDP -c $fnIN -p $fnTOP $fnNDX -o $baseOUT.tpr $(program_options grompp) -maxwarn -1"
    ...
    CMD="$MDRUN -nice 0 -deffnm $baseOUT -c $fnOUT -cpi $baseOUT.cpt -nt $NP $SPLIT $(program_options mdrun) -maxh $MAXH"
    echo
    echo "$CMD" | tee -a $fnLOG
    echo
    $CMD >>$fnLOG 2>&1 &
    local -i MDPID=$!
...
# 1. Basic stuff
PDB2GMX="$GMXBIN/pdb2gmx -v -f $dirn/$pdb -o $base.gro -p $base.top -ignh -ff $ForceField -water $SolModel"
...
then
    ...
    [[ -z $GRO && -f $fnIN ]] && editconf -f $fnIN -o $base.gro >/dev/null 2>&1
...
if $NDLP
then
    PBC="$GMXBIN/squeeze -f $GRO -s $GRO -o $base-pbc.gro -d $PBCDIST -c"
    tag=squeeze
else
    PBCDIST=$(python -c "print 0.5*$PBCDIST")
    PBC="$GMXBIN/editconf -f $GRO -o $base-pbc.gro -bt $BOXTYPE -d $PBCDIST -c"
    tag=editconf
fi
...
then
    ...
    GENBOX="$GMXBIN/genbox -cp $GRO -cs $SolFile -o $base-sol-b4ions.gro"
    ...
    #if [[ ! $(awk "$AWK_MOLTYPE" $TOP) =~ SOL ]] 
    if [[ ! $MOLTYPES =~ SOL ]] 
    then
	# check if a file is #included with the solvent model
	grep -q '#include.*'$SolModel'.itp' $TOP || $SED -i.bck '/^\[ *system *\]/s/^/#include "'${SolModel}.itp$'"\\\n\\\n/' $base-sol-b4ions.top
    fi
    ...
    tag="sol-b4ions"
    GROMPP="$GMXBIN/grompp -v -f empty.mdp -c $base-$tag.gro -p $base-$tag.top -o $base-$tag.tpr -po defaults.mdp -maxwarn -1"
	...
	then
	    ...
	    NDX="-n charge.ndx"
	    tpbconv -s $base-sol-b4ions.tpr -o $base-sol-b4ions-noligand.tpr -n charge.ndx >/dev/null 2>&1
	...
	then
	    ...
	    grep -q '#include.*ions.itp' $base-sol-b4ions.top || IONSITP=$'/^\[ *system *\]/s/^/#include "ions.itp"\\\n\\\n/;'
	fi
...
then
    ...
    then
        ...
        # First a basic one
	echo q | $GMXBIN/make_ndx -f $DIR/$base-sol.gro -o $base-sol.ndx >/dev/null 2>&1
...
then
    ...
    do
	echo $terms | g_energy -f $edr -o ${edr%.edr}.xvg 2>/dev/null | sed '/^Energy/,/^ *$/{/^ *$/q}' > ${edr%.edr}.lie
    done
fi
